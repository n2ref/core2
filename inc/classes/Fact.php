...
class Fact
{
    ...
    public function __construct()
    {
        ...
        $this->shm_id =
            ftok($eventFile, "t") +
            crc32($_SERVER["SERVER_NAME"] . strval($user_key)); //у аждого юзера своя очередь
        $this->shm_public =
            ftok($eventFile, "t") + crc32($_SERVER["SERVER_NAME"]); //общая очередь
    }
     ...
     * @return $this
     */
    public function __get($v): self
    {
        if (!isset($this->messages[$v])) {
            $this->messages[$v] = [];
        }
        $this->topic = $v;
        return $this;
    ...
    public function message(mixed $text, bool $is_public = false): void
    {
        ...
        $q = !$is_public
            ? msg_get_queue($this->shm_id)
            : msg_get_queue($this->shm_public);
        if (
            !msg_send(
                $q,
                self::TEXT,
                json_encode([$this->topic => $text]),
                false,
                true,
                $msg_err,
            )
    ...
    public function elementText(
        ...
        $text = json_encode([
            "element" => ["selector" => $selector, "text" => $text],
        ]);
        $q = !$is_public
            ? msg_get_queue($this->shm_id)
            : msg_get_queue($this->shm_public);
